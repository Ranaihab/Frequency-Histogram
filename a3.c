...
int main(int argc , char * argv [])
{
	...
	if(myid==0){
		myPoints = malloc((myPointNum+remainder) * sizeof(int));
	}
	else{
	...
	if(myid==0){
		j = myPointNum * numprocs;
	
		for(i = 0; i < remainder; i++){
			myPoints[myPointNum+i] = points[j];
			j++;
		}
		myPointNum+=remainder;
	}

	//for(i=0;i<myPointNum;i++){
		//printf("myid: %d point: %d\n", myid, myPoints[i]);
	//}
    ...
    for (i=0;i<myPointNum;i++){
		...
		//printf("process: %d thread: %d  point: %d bar: %d\n", myid, threadID, myPoints[i], j);
        #pragma omp critical
        countBars[j]++;
    }
